...
\begin{enumerate}[label=\textbf{\alph*)}]
	...
	\item O limite teórico para o ganho de performance (aceleração), de acordo com
		...
		Isso nos diz que a aceleração máxima de um programa que não tem uma parte estritamente sequencial
		(o que seria $T_s$) e utiliza 3 processadores vale 3.

		Entretanto, vale a pena olhar com mais cuidado para o caso que temos em mãos aqui. Como o valor que
		estamos tentando obter é um valor \emph{teórico}, vamos supor que as três threads que rodam paralelamente
		começam e terminam ao mesmo tempo. Vamos supor que cada thread leva o tempo $T' = T_P(1)/5$ (já que estamos
		dividindo a parte sequêncial em 5). Temos que o tempo para as três threads paralelas executarem é apenas $T'$,
		já que é o tempo gasto por cada uma individualmente e elas executam ao mesmo tempo (é o mesmo que ter uma
		aceleração 3 para uma seção que consome o tempo $3T'$!).\\
		Agora que três threads foram concluídas, restam duas para executar. De forma análoga, essas duas restantes
		gastarão apenas o tempo $T'$. Isso nos dá que $T_P(P)$ nesse caso vale $2T' = 2\frac{T_P(1)}{5}$ e a aceleração
		é $\frac{5}{2}$ (o que é inferior ao que obteriamos se utilizassemos apenas 3 threads!).

		No geral, podemos deduzir que a fórmula geral para o tempo consumido quando dividimos uma seção entre $n$
		threads, onde temos somente $P$ processadores (onde $n \ge P$), é:
		\begin{center}
			$T_P(n) = \lceil\frac{n}{P}\rceil\frac{T_P(1)}{n}$
		\end{center}
		Onde $\lceil x \rceil$ significa o menor inteiro maior ou igual a $x$ (função teto).
